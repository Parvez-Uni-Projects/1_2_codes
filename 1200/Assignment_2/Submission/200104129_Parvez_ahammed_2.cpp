...
void console_scanner()
{
    ...
    for (int i = 0 ; i < number_of_students ; i ++)
        {
            printf("");
            scanf("%s %d",&string[i].id,&details[i].id);
            
            printf("");
            scanf("%s %s",&string[i].name,&details[i].name);

            printf("");
            scanf("%s %f",&string[i].total_gpa,&details[i].total_gpa);
            total_gpa_arr[i] = details[i].total_gpa;

            printf("");
            scanf("%s %f",&string[i].hsc_gpa,&details[i].hsc_gpa);
            hsc_gpa_arr[i] = details[i].hsc_gpa;

            printf("");
            scanf("%s %f",&string[i].ssc_gpa,&details[i].ssc_gpa);
            ssc_gpa_arr[i] = details[i].ssc_gpa;
...
void file_data_appender()
{
    FILE *student_file = fopen("student_data.txt","a");

    for (int i =0 ; i < number_of_students ; i ++)
    {
        fprintf (student_file,"%s\t\t%d\n",string[i].id,details[i].id);
        fprintf (student_file,"%s\t\t%s\n",string[i].name,details[i].name);
        fprintf (student_file,"%s\t%.2f\n",string[i].total_gpa,details[i].total_gpa);
        fprintf (student_file,"%s\t\t%.2f\n",string[i].hsc_gpa,details[i].hsc_gpa);
        fprintf (student_file,"%s\t\t%.2f\n\n",string[i].ssc_gpa,details[i].ssc_gpa);
...
void function_that_does_everything(){
...
{ 
    ...
    printf("How many stdent data do you want to analysis ? \n");
    scanf("%d",&number_of_students);

    int choice;
    printf("Choice for data\n1. Read from the file \n2. Input from the console \n\nEnter your choice\n");
    scanf("%d",&choice);

    switch (choice)
    {
    case 1:         if (student_file == NULL){              //checking if the pointer contains anything if it does scanning will be done 
